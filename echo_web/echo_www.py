...
while True: # just keep serving page to any client that connects
    ...
    req = client.recv(size) # HTTP request - not too big!  Just ignore contents
    print req
    path = req.split()[1] # split request to get path part of the url
    print path
    client.send(page % path[1:]) # HTTP response - same for any request
    client.close()
